...
// Gatekeeper handles all auth checks. If we are here, we are authorized.

$txn = filter_input(INPUT_GET, 'txn');
$net = filter_input(INPUT_GET, 'net');
$dev = filter_input(INPUT_GET, 'dev');

if( $dev || $txn ) { $db = new Database(); }

if( $dev == 'all' ) echo json_encode($db->get_devices(), JSON_PRETTY_PRINT);
if( $dev == 'active' ) echo json_encode($db->get_active_devices(), JSON_PRETTY_PRINT);
if( $dev == 'restricted' ) echo json_encode($db->get_restricted_devices());
if( $dev == 'recent' ) echo json_encode($db->get_recent_devices(), JSON_PRETTY_PRINT);

if( $dev == 'add_session' ) {
  $limit = filter_input(INPUT_GET, 'limit', FILTER_VALIDATE_INT);
  $mac = filter_input(INPUT_GET, 'mac', FILTER_VALIDATE_MAC);
  if( !$limit ) exit;
  $db->set_mac($mac); $db->set_mb_limit($limit);
  if( !$db->get_device_id() ) { http_response_code(403); exit; }
  $db->add_session();
  list($mb_limit,$mb_used) = $db->get_data_usage();
  if( $mb_limit > $mb_used ) {
    $ipt = new Iptables($db->get_device_ip()); $ipt->add_client();
  }
